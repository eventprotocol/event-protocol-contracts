...
contract('Event Token Unit Testing', async (accounts) => {
  ...
  beforeEach(async function(){
      ...
      instance.resume.call();
  });

  it("Test 1: Expected total supply of tokens: 5000000", async() => {
      return instance.totalSupply.call().then(function(totalSupply) {
        assert.strictEqual(totalSupply.toNumber(), 5000000*Math.pow(10, 18), "5000000 is not the total supply");
    });
  });
  ...
  it("Test 9: Transfer 1000 ET from accounts[0] to accounts[1] -> Transfer 500 tokens from accounts[1] to accounts[2]", async () => {
  ...
  it("Test 10: Expected allowances should be 1000 and 2000", async() => {
      ...
      let _amount2 = 2000*Math.pow(10,18);
      ...
      assert.equal(allowance1.toNumber(), _amount1, "The allowances for accounts[0] and accounts[1] do not match");
      assert.equal(allowance2.toNumber(), _amount2, "The allowances for accounts[1] and accounts[2] do not match");
